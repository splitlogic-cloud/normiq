...
async function searchLibrary(question: string): Promise<{
  answer: string; sources: string[]; risk_level: string; library_id: string
} | null> {
  try {
    ...
    if (bestScore >= LIBRARY_THRESHOLD && bestMatch) {
      await supabaseAdmin
        .from('library')
        .update({ use_count: (bestMatch.use_count || 0) + 1 })
        .eq('id', bestMatch.id)
      return { answer: bestMatch.answer, sources: bestMatch.sources || [], risk_level: bestMatch.risk_level || 'LÅG', library_id: bestMatch.id }
    }
    return null
  } catch { return null }
}
...
// ── RISK-KLASSNING ────────────────────────────────────────────────────────
function classifyRisk(question: string, sources: { ref: string; text: string }[]): { level: 'LÅG' | 'MEDEL' | 'HÖG'; reason: string } {
  const q = question.toLowerCase()
  const hogSignals = ['fåmansbolag', '3:12', 'kvalificerad andel', 'verksamhet i betydande', 'underprisöverlåtelse', 'förtäckt utdelning', 'skatteflykt', 'genomsyn', 'internprissättning', 'cfc', 'utflyttning', 'fusion', 'likvidation', 'omstrukturering', 'generationsskifte']
  const medelSignals = ['representation', 'förmån', 'bilförmån', 'tjänstebil', 'hemkontor', 'periodisering', 'inkurans', 'nedskrivning', 'koncernbidrag', 'ränteavdrag', 'kapitalvinst', 'uppskov', 'rot', 'rut', 'traktamente', 'milersättning', 'uthyrning', 'dubbel bosättning']
  if (hogSignals.some(s => q.includes(s))) return { level: 'HÖG', reason: 'Frågan berör ett område med hög komplexitet och individuella bedömningar. Konsultera en skatteexpert.' }
  if (medelSignals.some(s => q.includes(s))) return { level: 'MEDEL', reason: 'Tydliga regler men med vanliga undantag. Verifiera att grundförutsättningarna stämmer i ditt fall.' }
  const sourceTexts = sources.map(s => s.text.toLowerCase()).join(' ')
  if (sourceTexts.includes('beroende på') || sourceTexts.includes('om') || sourceTexts.includes('kan')) return { level: 'MEDEL', reason: 'Regelverket innehåller villkor som kan variera. Stäm av mot din specifika situation.' }
...
function needsWebSearch(question: string): boolean {
  ...
  const triggers = [
    ...
    'rot', 'rut', 'schablon', 'kostförmån', 'julklapp', 'jubileumsgåva',
    'minnesgåva', 'direktavdrag', 'kompletteringsregeln', 'uppskov', 'uthyrning',
    // 3:12 och utdelning — nya regler 2026
    '3:12', 'fåmansbolag', 'fåmansföretag', 'utdelning', 'lönebaserat', 'gränsbelopp',
    // Tidskänsliga
    '2026', 'i år', 'aktuell', 'nuvarande', 'gäller nu', 'aktuellt belopp', 'nya regler', 'ny regel',
  ]
  return triggers.some(s => q.includes(s))
}
...
// ── KÄLLVALIDERING ────────────────────────────────────────────────────────
// Välkända skattesatser och procenttal som aldrig ska flaggas
const KNOWN_RATES = new Set(['0', '6', '12', '20', '21', '22', '25', '28', '30', '50', '57', '65', '66', '100'])
...
function findUnsupportedNumbers(answer: string, sources: { text: string }[]): string[] {
  ...
  const unsupported = found.filter(pct => {
    if (KNOWN_RATES.has(pct)) return false // Aldrig flagga kända skattesatser
    return !new RegExp(`${pct}\\s*%`).test(sourceText)
  })
...
export async function POST(req: Request) {
  ...
  if (!allowed) {
    return Response.json({ content: 'Du har ställt för många frågor på kort tid. Vänta en minut och försök igen.' }, { status: 429 })
  }

  // ── Steg 1: Kunskapsbiblioteket ──────────────────────────────────────────
  if (!useWebSearch) {
    const libraryHit = await searchLibrary(lastQuestion)
    if (libraryHit) {
      const { data: inserted } = await supabase
        .from('queries')
        .insert({ question: lastQuestion, answer: libraryHit.answer, sources: libraryHit.sources, risk_level: libraryHit.risk_level, session_id: sessionId || 'anonymous' })
        .select('id').single()
      return Response.json({ content: libraryHit.answer, sources: libraryHit.sources, risk_level: libraryHit.risk_level, query_id: inserted?.id, verified: true, from_library: true })
    }
  }

  // ── Steg 2: In-memory cache ───────────────────────────────────────────────
  if (!useWebSearch) {
    const cached = getCached(lastQuestion)
    if (cached) {
      return Response.json({ content: cached.answer, sources: cached.sources, risk_level: cached.risk_level, query_id: null, verified: true, from_cache: true })
    }
  }

  // ── Steg 3: Vektorsökning ─────────────────────────────────────────────────
  ...
  try {
    ...
    if (vectorResults && vectorResults.length > 0 && vectorResults[0].similarity > 0.2) {
      const boosted = vectorResults
        .map((r: { content: string; metadata: { ref: string; rubrik: string; lag?: string }; similarity: number }) => ({ ...r, boostedScore: boostScore(r) }))
        .sort((a: { boostedScore: number }, b: { boostedScore: number }) => b.boostedScore - a.boostedScore)
        .slice(0, 14)
      retrievedSources = boosted.map((r: { content: string; metadata: { ref: string; rubrik: string }; similarity: number }) => ({
...
DIN UPPGIFT:
Förklara vad källtexterna och/eller webbsökningens resultat faktiskt säger. Inte vad du tror eller minns.

═══════════════════════════════════════════════════════
ABSOLUTA REGLER — BRYTS ALDRIG:

1. ANVÄND BARA SIFFROR FRÅN KÄLLORNA
   Procentsatser, belopp och gränsvärden får ENDAST hämtas från källtexterna
   eller från web_search. Aldrig från eget minne.

2. OM KÄLLAN SAKNAS — SÄG DET
   Om källtexterna inte täcker frågan: en mening + hänvisa till skatteverket.se.

3. INGA PÅHITTADE REGELÄNDRINGAR
   Ange aldrig att en regel har ändrats om det inte finns i källan.

4. VÄLKÄNDA SKATTESATSER ÄR ALLTID OK
   Dessa behöver inte finnas i källtexterna: 0%, 6%, 12%, 20%, 25%, 30%, 50-66%.
   De är lagstadgade grundskattesatser och aldrig fel att nämna i rätt sammanhang.
═══════════════════════════════════════════════════════

TILLGÄNGLIGA KÄLLOR:
${källkontext}

${usedFallback ? 'OBS: Källorna kommer från det manuella regelindexet.' : ''}${bokforingExtra}${webSearchInstruktion}

SVARSFORMAT:

## [Rubrik]

[Svar baserat på källtexterna/webbsökning. Ange exakta lagrum [IL 57 kap. 10 §].]

---FÖRENKLAT---
Enkelt uttryckt: [4–7 meningar utan juridisk jargong.]
...
REGLER:
- Ge alltid ett komplett svar
- Risk-raden är ALLTID sista raden
- Svara på svenska
- Om osäker: en mening + hänvisa till skatteverket.se`
  ...
  try {
    ...
    const isOverloaded = (err as { status?: number })?.status === 529
    return Response.json({ content: isOverloaded ? 'Normiq är just nu under hög belastning. Försök igen.' : 'Ett oväntat fel uppstod.' }, { status: 503 })
  }
  ...
  // Källvalidering — bara flagga okända siffror, inte välkända skattesatser
  ...
  if (hasUnsupportedClaims) {
    finalAnswer = answer + `\n\n⚠️ _Obs: Svaret innehåller värden (${unsupportedNumbers.map(n => n + '%').join(', ')}) som inte kunde verifieras mot källtexterna. Kontrollera mot [Skatteverket](https://skatteverket.se)._`
  } else if (!verified && !useWebSearch) {
  ...
  try {
    const { data: inserted } = await supabase
      .from('queries')
      .insert({ question: lastQuestion, answer: finalAnswer, sources: sourceRefs, risk_level: risk.level, session_id: sessionId || 'anonymous' })
      .select('id').single()

    return Response.json({ content: finalAnswer, sources: sourceRefs, risk_level: risk.level, query_id: inserted?.id, verified, web_search_used: useWebSearch })
  } catch {
    return Response.json({ content: finalAnswer, sources: sourceRefs, risk_level: risk.level, query_id: null, verified, web_search_used: useWebSearch })
  }
}