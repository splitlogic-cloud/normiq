...
function parseReceipt(text: string): { description: string; amount: string; vat: string } {
  ...
  for (const pattern of amountPatterns) {
    for (const line of lines) {
      ...
      if (match) { amount = match[1].replace(/\s/g, '').replace(',', '.'); break }
    }
    if (amount) break
  }
  if (!amount) {
  ...
  const descLine = lines.find(l => l.length > 3 && !/^\d{4}-\d{2}-\d{2}/.test(l) && !/^(\d[\d\s.,]+)\s*(kr|sek|:-)?$/i.test(l) && !/^(org\.?nr|moms\.?nr|kvitto|receipt|faktura)/i.test(l)) || lines[0] || ''
  return { description: descLine, amount, vat }
}

type Message = { role: 'user' | 'assistant'; content: string; queryId?: string; feedback?: 1 | -1 | null }
type HistoryItem = { id: string; question: string; answer: string; sources: string[]; risk_level: string; created_at: string }
type AnalysisResult = { account: string; account_name: string; vat_account: string; vat_amount: number; net_amount: number; deductible: boolean; deductible_percent: number; depreciation_years: number | null; risk: 'LÅG' | 'MEDEL' | 'HÖG'; risk_reason: string; lagrum: string[]; confidence: number; reasoning: string; warning?: string | null }
type AnalysisHistoryItem = { description: string; amount: number; result: AnalysisResult; created_at: string }
...
function parseAnswer(content: string) {
  if (!content || typeof content !== 'string') return { body: 'Ett fel uppstod. Försök igen.', simplified: '', example: '', sources: '', riskLine: '' }
  ...
  const body = simplifiedParts[0].split('\n').filter(l => l.trim() !== sourceLine.trim() && l.trim() !== riskLine.trim()).join('\n').trim()
  const simplified = simplifiedParts[1] ? simplifiedParts[1].split('---EXEMPEL---')[0].split('\n').filter(l => !l.toLowerCase().startsWith('källor:') && !l.toLowerCase().startsWith('risk:')).join('\n').trim() : ''
  const example = exampleParts[1] ? exampleParts[1].split('\n').filter(l => !l.toLowerCase().startsWith('källor:') && !l.toLowerCase().startsWith('risk:')).join('\n').trim() : ''
...
function renderInline(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) return <strong key={i} style={{ color: '#0A0A0C', fontWeight: 600 }}>{part.slice(2, -2)}</strong>
    if (part.startsWith('[') && part.endsWith(']')) {
      const inner = part.slice(1, -1); const law = inner.split(' ')[0]; const url = LAG_URLS[law]
      const badgeStyle: React.CSSProperties = { color: '#C0321A', textDecoration: 'none', fontFamily: 'DM Mono, monospace', fontSize: '0.84em', border: '1px solid rgba(192,50,26,.25)', borderBottom: '2px solid rgba(192,50,26,.45)', padding: '2px 7px 2px 5px', borderRadius: 3, whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 3 }
      return url ? <a key={i} href={url} target="_blank" rel="noopener noreferrer" style={badgeStyle} onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'rgba(192,50,26,.07)' }} onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'transparent' }}><span style={{ opacity: 0.65, fontSize: '0.9em' }}>§</span>{inner}</a> : <span key={i} style={{ ...badgeStyle, cursor: 'default' }}><span style={{ opacity: 0.65, fontSize: '0.9em' }}>§</span>{inner}</span>
    }
    return <span key={i}>{part}</span>
  })
}

function isTableBlock(lines: string[]): boolean { return lines.length >= 2 && lines[0].includes('|') && lines[1].replace(/[\s|:-]/g, '') === '' }

function renderTable(lines: string[], key: number): React.ReactNode {
  const rows = lines.filter((_, i) => i !== 1).map(l => l.split('|').map(c => c.trim()).filter((_, i, arr) => i > 0 && i < arr.length - 1))
  const [header, ...body] = rows
  return (
    <div key={key} style={{ overflowX: 'auto', margin: '12px 0 18px' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead><tr style={{ borderBottom: '1.5px solid #0A0A0C' }}>{header.map((h, i) => <th key={i} style={{ padding: '9px 16px', textAlign: 'left', fontFamily: 'DM Mono, monospace', fontWeight: 500, color: '#0A0A0C', fontSize: 11, letterSpacing: '.07em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>)}</tr></thead>
        <tbody>{body.map((row, ri) => <tr key={ri} style={{ borderBottom: '1px solid #F0EDE6', background: ri % 2 === 0 ? 'white' : '#FAFAF8' }}>{row.map((cell, ci) => <td key={ci} style={{ padding: '10px 16px', fontFamily: 'Georgia, serif', fontSize: 14, color: ci === 0 ? '#333' : '#555', lineHeight: 1.65 }}>{renderInline(cell)}</td>)}</tr>)}</tbody>
      </table>
    </div>
  )
}
...
function formatBody(text: string) {
  if (!text) return null
  const lines = text.split('\n'); const elements: React.ReactNode[] = []; let key = 0; let i = 0
  while (i < lines.length) {
    ...
    if (trimmed.startsWith('## ')) { elements.push(<h2 key={key++} style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 26, fontWeight: 600, color: '#0A0A0C', margin: '4px 0 14px', lineHeight: 1.1, letterSpacing: '-.01em' }}>{trimmed.slice(3)}</h2>); i++; continue }
    if (trimmed.startsWith('### ')) { elements.push(<h3 key={key++} style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 20, fontWeight: 600, color: '#0A0A0C', margin: '14px 0 8px', lineHeight: 1.2 }}>{trimmed.slice(4)}</h3>); i++; continue }
    if (trimmed.startsWith('- ') || trimmed.startsWith('• ')) { elements.push(<div key={key++} style={{ display: 'flex', gap: 10, marginBottom: 6, alignItems: 'flex-start' }}><span style={{ color: '#C0321A', flexShrink: 0, marginTop: 5, fontSize: 10, fontFamily: 'DM Mono, monospace' }}>→</span><span style={{ fontFamily: 'Georgia, serif', fontSize: 15, color: '#333', lineHeight: 1.8 }}>{renderInline(trimmed.replace(/^[-•]\s+/, ''))}</span></div>); i++; continue }
    if (/^\d+\.\s/.test(trimmed)) { const num = trimmed.match(/^(\d+)\./)?.[1]; elements.push(<div key={key++} style={{ display: 'flex', gap: 10, marginBottom: 6, alignItems: 'flex-start' }}><span style={{ fontFamily: 'DM Mono, monospace', color: '#C0321A', flexShrink: 0, fontSize: 11, marginTop: 4, minWidth: 18 }}>{num}.</span><span style={{ fontFamily: 'Georgia, serif', fontSize: 15, color: '#333', lineHeight: 1.8 }}>{renderInline(trimmed.replace(/^\d+\.\s/, ''))}</span></div>); i++; continue }
    elements.push(<p key={key++} style={{ fontFamily: 'Georgia, serif', fontSize: 15, color: '#1a1a1a', lineHeight: 1.9, marginBottom: 10 }}>{renderInline(trimmed)}</p>); i++
  }
  return <>{elements}</>
}
...
function timeAgo(dateStr: string) {
  ...
  if (mins < 2) return 'Just nu'; if (mins < 60) return `${mins} min sedan`; if (hours < 24) return `${hours} tim sedan`; if (days < 7) return `${days} d sedan`
...
export default function App() {
  ...
  function loadFromHistory(item: HistoryItem) {
    setMessages([{ role: 'user', content: item.question }, { role: 'assistant', content: item.answer, queryId: item.id, feedback: null }])
  }
  ...
  async function runAnalysis(desc?: string, amt?: string) {
    const d = desc !== undefined ? desc : analyzeDesc; const a = amt !== undefined ? amt : analyzeAmount
    if (!d.trim() || !a) return
    setAnalyzeLoading(true); setAnalyzeError(''); setAnalyzeResult(null)
    try {
      const res = await fetch('/api/tax-analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ description: d, amount: parseFloat(a), vat_rate: parseInt(analyzeVat), vat_included: true, entity_type: analyzeEntity }) })
      const data = await res.json()
      if (data.error) { setAnalyzeError(data.error); return }
      setAnalyzeResult(data); await loadAnalysisHistory()
    } catch { setAnalyzeError('Ett fel uppstod. Försök igen.') }
  ...
  async function handleFeedback(msgIndex: number, rating: 1 | -1) {
    ...
    const { data: { user } } = await supabase.auth.getUser()
    await fetch('/api/feedback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query_id: msg.queryId, question, answer: msg.content, rating, session_id: user?.id || sessionId }) })
  }
    ...
    <div style={{ display: 'flex', height: '100vh', background: '#F5F3EE', fontFamily: 'Georgia, serif' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .send-btn.active { background: #C0321A; } .send-btn.active:hover { background: #A02818; transform: translateY(-1px); } .send-btn:disabled { opacity: .3; cursor: not-allowed; transform: none; }
          ...
          <>
            <div style={{ flex: 1, overflowY: 'auto', padding: '36px 48px' }}>
              {messages.length === 0 ? (
                ...
                <div style={{ maxWidth: 800, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 28 }}>
                        ...
                        <div style={{ background: 'white', border: '1px solid #E0DDD6', borderRadius: '4px 12px 12px 12px', overflow: 'hidden' }}>
                          ...
                          <div style={{ borderTop: '1px solid #F0EDE6', padding: '12px 28px', background: 'white' }}>
                                ...
                                {sources.replace(/^källor:\s*/i, '').split(',').map((s, idx) => {
                                  ...
                                  return (
                                    <span key={idx} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                      ...
                                      {url ? <a href={url} target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'DM Mono, monospace', fontSize: 11, color: '#C0321A', textDecoration: 'none', border: '1px solid rgba(192,50,26,.2)', borderBottom: '2px solid rgba(192,50,26,.4)', padding: '2px 7px 2px 5px', borderRadius: 3, display: 'inline-flex', alignItems: 'center', gap: 3, transition: 'background .15s' }} onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'rgba(192,50,26,.07)' }} onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'transparent' }}><span style={{ opacity: 0.6, fontSize: '0.85em' }}>§</span>{trimmed}</a> : <span className="mono" style={{ fontSize: 11, color: '#888' }}>{trimmed}</span>}
                                    </span>
                                  )
                                })}
                              </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                              {riskLine ? <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ width: 5, height: 5, borderRadius: '50%', background: rc.text, flexShrink: 0, display: 'inline-block', opacity: 0.7 }} /><span className="mono" style={{ fontSize: 10, color: '#AAA', letterSpacing: '.04em' }}>{riskLine.replace(/^risk:\s*/i, '')}</span></div> : <div />}
                  ...
                  {loading && <div className="msg-in"><div style={{ background: 'white', border: '1px solid #E0DDD6', borderRadius: '4px 12px 12px 12px', padding: '18px 26px', display: 'inline-block' }}><div className="typing"><span/><span/><span/></div></div></div>}
                  <div ref={bottomRef}/>
                </div>
              )}
            </div>
            <div style={{ padding: '14px 48px 20px', background: '#F5F3EE', borderTop: '1px solid #E0DDD6' }}>
              <div style={{ maxWidth: 800, margin: '0 auto' }}>
                ...
                <div className="mono" style={{ fontSize: 10, color: '#CCC', textAlign: 'center', marginTop: 9, letterSpacing: '.04em' }}>Baseras på IL · ML · BFL · SFL · ABL — verifiera alltid med en skatteexpert</div>
              </div>
            </div>
          </>
        )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 14 }}>
                ...
                {inputMode === 'fritext' && (
                  <div className="field-wrap">
                    ...
                    <input value={analyzeDesc} onChange={e => setAnalyzeDesc(e.target.value)} placeholder="T.ex. MacBook Air, restaurang klientmöte, Slack..." style={{ width: '100%', border: 'none', background: 'transparent', fontFamily: 'Georgia, serif', fontSize: 15, color: '#1a1a1a' }} />
                  </div>
                )}
                {inputMode === 'kvitto' && (
                  <div>
                    ...
                    {receiptText.trim() && !receiptParsed && (
                      <button onClick={() => { const parsed = parseReceipt(receiptText); if (parsed.description) setAnalyzeDesc(parsed.description); if (parsed.amount) setAnalyzeAmount(parsed.amount); if (parsed.vat) setAnalyzeVat(parsed.vat); setReceiptParsed(true) }}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 10 }}>
                  <div className="field-wrap">
                    ...
                    <input value={analyzeAmount} onChange={e => setAnalyzeAmount(e.target.value)} placeholder="24 990" type="number" style={{ width: '100%', border: 'none', background: 'transparent', fontFamily: 'DM Mono, monospace', fontSize: 15, color: '#1a1a1a' }} />
                  </div>
                  <div className="field-wrap">
                    <div className="mono" style={{ fontSize: 9, letterSpacing: '.12em', textTransform: 'uppercase', color: '#BBB', marginBottom: 5 }}>Moms %</div>
                    <select value={analyzeVat} onChange={e => setAnalyzeVat(e.target.value)} style={{ width: '100%', border: 'none', background: 'transparent', fontFamily: 'DM Mono, monospace', fontSize: 14, color: '#1a1a1a', cursor: 'pointer' }}>
                  ...
                  <div className="field-wrap">
                    ...
                    <select value={analyzeEntity} onChange={e => setAnalyzeEntity(e.target.value)} style={{ width: '100%', border: 'none', background: 'transparent', fontFamily: 'DM Mono, monospace', fontSize: 14, color: '#1a1a1a', cursor: 'pointer' }}>
                ...
                {inputMode === 'kvitto' && (
                  <div className="field-wrap">
                    ...
                    <input value={analyzeDesc} onChange={e => setAnalyzeDesc(e.target.value)} placeholder="Extraheras automatiskt från kvittot..." style={{ width: '100%', border: 'none', background: 'transparent', fontFamily: 'Georgia, serif', fontSize: 15, color: '#1a1a1a' }} />
                  </div>
                )}
              </div>
                ...
                <div className="result-in" style={{ marginTop: 24 }}>
                  <div style={{ background: 'white', border: '1px solid #E0DDD6', borderRadius: 10, overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '16px 26px', borderBottom: '1px solid #F0EDE6', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                      <div><div className="mono" style={{ fontSize: 9, letterSpacing: '.12em', textTransform: 'uppercase', color: '#CCC', marginBottom: 5 }}>Avdragsrätt</div><div style={{ fontFamily: 'Georgia, serif', fontSize: 14, color: analyzeResult.deductible ? '#2E6644' : '#A02818' }}>{analyzeResult.deductible ? `Ja — ${analyzeResult.deductible_percent}%` : 'Nej'}</div></div>
                      <div><div className="mono" style={{ fontSize: 9, letterSpacing: '.12em', textTransform: 'uppercase', color: '#CCC', marginBottom: 5 }}>Avskrivning</div><div style={{ fontFamily: 'Georgia, serif', fontSize: 14, color: '#333' }}>{analyzeResult.depreciation_years ? `${analyzeResult.depreciation_years} år` : 'Direktavdrag'}</div></div>
                      <div><div className="mono" style={{ fontSize: 9, letterSpacing: '.12em', textTransform: 'uppercase', color: '#CCC', marginBottom: 5 }}>Säkerhet</div><ConfidenceDots value={analyzeResult.confidence} /></div>
                    </div>
                    ...
                    <div style={{ padding: '12px 26px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        {analyzeResult.lagrum.map((l, i) => {
                          ...
                          return <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>{i > 0 && <span style={{ color: '#E0DDD6' }}>·</span>}{url ? <a href={url} target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'DM Mono, monospace', fontSize: 11, color: '#C0321A', textDecoration: 'none', border: '1px solid rgba(192,50,26,.2)', borderBottom: '2px solid rgba(192,50,26,.4)', padding: '2px 7px 2px 5px', borderRadius: 3, display: 'inline-flex', alignItems: 'center', gap: 3 }}><span style={{ opacity: 0.6, fontSize: '0.85em' }}>§</span>{l}</a> : <span className="mono" style={{ fontSize: 11, color: '#888' }}>{l}</span>}</span>
                        })}
                      </div>
                      {(() => { const rc = getRiskColor(analyzeResult.risk); return <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ width: 5, height: 5, borderRadius: '50%', background: rc.text, display: 'inline-block', opacity: 0.7 }} /><span className="mono" style={{ fontSize: 10, color: '#AAA' }}>{analyzeResult.risk} — {analyzeResult.risk_reason}</span></div> })()}
                    </div>
                  </div>
                </div>
              )}
              {analyzeLoading && <div style={{ marginTop: 24, background: 'white', border: '1px solid #E0DDD6', borderRadius: 10, padding: '22px 26px', display: 'inline-block' }}><div className="typing"><span/><span/><span/></div></div>}
            </div>
          </div>
        )}
      </div>
